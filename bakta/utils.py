...
DEPENDENCY_DIAMOND = (Version(2, 2, 0), Version(VERSION_MAX_DIGIT, VERSION_MAX_DIGIT, VERSION_MAX_DIGIT), VERSION_REGEX, 'Diamond', ('diamond', 'help'), ['--skip-cds', '--skip-sorf'])
...
def parse_arguments():
    ...
    arg_group_organism.add_argument('--domain', action='store', default=bc.DOMAIN_BACTERIA, choices=[bc.DOMAIN_BACTERIA, bc.DOMAIN_ARCHAEA], help='Organism domain profile: bacteria/archaea (default = bacteria)')
    ...
    arg_group_annotation.add_argument('--gram', action='store', default=bc.GRAM_UNKNOWN, choices=[bc.GRAM_POSITIVE, bc.GRAM_NEGATIVE, bc.GRAM_UNKNOWN], help=f'Gram type for signal peptide predictions: {bc.GRAM_POSITIVE}/{bc.GRAM_NEGATIVE}/{bc.GRAM_UNKNOWN} (default = {bc.GRAM_UNKNOWN}; ignored for --domain archaea)')
...
def test_dependencies():
    ...
    if(cfg.skip_cds is not None and cfg.skip_cds is False):
        ...
        if(cfg.domain == bc.DOMAIN_BACTERIA):
            ...
            process = sp.run(['amrfinder', '--debug', '--database', str(amrfinderplus_db_latest_path)], capture_output=True)
    ...
    if(cfg.skip_ori is not None and cfg.skip_ori is False):
        if(cfg.domain == bc.DOMAIN_ARCHAEA):
            for executable in ['OriFinderArch', 'fimo', 'hmmsearch']:
                if(shutil.which(executable) is None):
                    sys.exit(f'ERROR: Archaeal oriC prediction requires {executable} on PATH. Install Ori-Finder-Arch, MEME Suite, and HMMER, or use --skip-ori.')
        else:
